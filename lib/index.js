...
function makeReducer(initialState) {
  ...
  */
	function makeActionCreator(type) {
		...
		var hasMeta = isFunction(metaReducer);
...
function makePrefixMapReducer() {
	...
	var map = reducers.reduce(function (a, r, i) {
		// TODO warning about foreign reducers
		var p = isFunction(r.getPrefix) ? r.getPrefix() : '@@FOREIGN' + i;
		if (!p) {
			return a;
		}
		if (p.charAt(p.length - 1) === '/') {
			return _extends({}, a, _defineProperty({}, p.substr(0, p.length - 1), r));
		}
		return _extends({}, a, _defineProperty({}, p, r));
	}, {});
